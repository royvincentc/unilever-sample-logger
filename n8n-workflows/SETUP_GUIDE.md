...
for (const item of existingRows) {
  ...
  // Find first row where CONTROL # is pre-filled but SAMPLE is empty
...
if (!controlNumber) {
  // Use the control number generated by the webapp if available (restores correct cross-month sequence)
  if (inputData.controlNumber) {
